...
foreach ($product as $k => $v) {
    ...
    $_code = $v['_code'];
    $_weight = $v['_weight'];
    $_name_distributor = $v['_name_distributor'];
    $_min_order = $v['_min_order'];

    $_content = $v['_content'];
    $_spesifikasi = $v['_spesifikasi'];
    ...
    $(document).ready(function () {
        $('body').on('click', '.btn-showmodal', function () {
            if (!$.cookie('signin'))
            {
                ...
                $.post('<?php echo URL; ?>order/formorder', {
                    product: '<?= $_code ?>'
                }).done(function (data) {
        ...
        <div class="col-9">
            <div class="row section">
                <div class="col-1">
                    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        <a data-toggle="pill"  href="#p0">
                            <img style="border:2px solid #d2d2d2;border-radius:5px;;margin-bottom: 5px;" class="img-fluid" src="<?= PATH_IMAGE_DISTRIBUTOR ?>/<?= $_picture ?>?a=<?= time() ?>" onerror="this.src='<?= PATH_IMAGE ?>logo.png?a=<?= time() ?>';" />
                            <?php
                            $tmpseq = 0;
                            if (count($_photos) > 0) {
                                ...
                                foreach ($_photos as $k => $v) {
                                    ...
                                    <a data-toggle="pill"  href="#p<?= $tmpseq ?>">
                                        <img style="border:2px solid #d2d2d2;border-radius:5px;;margin-bottom: 5px;" class="img-fluid" src="<?= PATH_IMAGE_DISTRIBUTOR ?>/image_detail/<?= $_detail_picture ?>?a=<?= time() ?>" onerror="this.src='<?= PATH_IMAGE ?>logo.png?a=<?= time() ?>';" />
                ...
                <div class="col-5" >
                    <div class="tab-content tab-image" id="v-pills-tabContent" style="border:0px solid #e2e7e9;width: 300px;height: 300px;">
                        ...
                        <div class="tab-pane fade show active" id="p0" role="tabpanel" aria-labelledby="v-pills-home-tab">
                            ...
                            <img style="width: 100%;height: 100%;border:2px solid #d2d2d2;border-radius:5px;;margin-bottom: 5px;" class="img-fluid" src="<?= PATH_IMAGE_DISTRIBUTOR ?>/<?= $_picture ?>?a=<?= time() ?>" onerror="this.src='<?= PATH_IMAGE ?>logo.png?a=<?= time() ?>';"  title="kategory 1"/>
                        ...
                        if (count($_photos) > 0) {
                            ...
                            foreach ($_photos as $k => $v) {
                                ...
                                <div class="tab-pane fade " id="p<?= $tmpseq ?>" role="tabpanel" aria-labelledby="v-pills-home-tab">
                                    <img style="width: 100%;height: 100%;border:2px solid #d2d2d2;border-radius:5px;;margin-bottom: 5px;" class="img-fluid" src="<?= PATH_IMAGE_DISTRIBUTOR ?>/image_detail/<?= $_detail_picture ?>?a=<?= time() ?>" onerror="this.src='<?= PATH_IMAGE ?>logo.png?a=<?= time() ?>';" />
                ...
                <div class="col-6 product-info">
                    ...
                    <div class="supplier-title"><?= $_name_distributor ?></div>
                    <div class="d-product-title"><?= $_name_product ?></div>
                    <div class="d-product-sale-price"><?= $_discount_price ?></div>
                    <div class="d-product-price">Minimum Order <?= $_min_order;?> pcs</div>
                    <!--
...
<?php
                        if (count($_trans) == 0 || $_trans[0]['_sts'] == '99' || $_trans[0]['_sts'] == '7') {
?>
                            ...
                            <div style="text-align: right;">
                                <a href="#" data="<?= $_code ?>" class="btn btn-cart btn-showmodal" >Order Barang</a>
                            </div>
<?php
                        }
?>
            ...
            <div class="row section">
                <div class="col-12">
                    <ul class="nav nav-pills mb-3 i-tab" id="pills-tab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="pills-home-tab" data-toggle="pill" href="#pills-home" role="tab" aria-controls="pills-home" aria-selected="true">Detil Pemesanan</a>
                        </li>

                        <li class="nav-item">
                            <a class="nav-link" id="pills-produk-tab" data-toggle="pill" href="#pills-produk" role="tab" aria-controls="pills-produk" aria-selected="true">Detil Produk</a>
                        </li>
        ...
        <div class="col-3">
            <div class="panel-container">
                ...
                <div class="panel-supplier">
                    ...
                    <!--<div class="supplier-btn"><a class="btn btn-1" href="<?= URL ?>supplier/index?a=<?= $_distributor ?>" role="button">Masuk Ke Toko</a></div>
